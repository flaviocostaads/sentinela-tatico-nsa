...
-- Add audit trigger for location data access (for compliance)
CREATE OR REPLACE FUNCTION public.audit_location_access()
RETURNS TRIGGER AS $$
BEGIN
  -- Log when admins/operators access location data
  IF auth.uid() != NEW.user_id THEN
    INSERT INTO public.audit_logs (
      ...
      auth.uid(),
      COALESCE((SELECT name FROM public.profiles WHERE user_id = auth.uid()), 'Unknown'),
      'LOCATION_ACCESS',
      'user_locations',
      NEW.id,
      jsonb_build_object(
        'accessed_user_id', NEW.user_id,
        'location_time', NEW.recorded_at,
        'lat', NEW.lat,
        'lng', NEW.lng
      )
    );
...
-- Apply audit trigger to track location data access
DROP TRIGGER IF EXISTS audit_location_access_trigger ON public.user_locations;
CREATE TRIGGER audit_location_access_trigger
  AFTER SELECT ON public.user_locations
  FOR EACH ROW
  WHEN (auth.uid() IS NOT NULL AND pg_trigger_depth() = 0)
  EXECUTE FUNCTION public.audit_location_access();